...
2) Удаляет записи, где отправитель — перекуп (BLOCKED_SENDERS).
3) Удаляет записи, где текст сообщения слишком длинный (списки с ценами).
4) Удаляет записи, где текст в формате списка перекупов (номера-цены, «СПИСОК», «ЦЕНА БЕЗ ОФОРМЛЕНИЯ»).
...
def main() -> None:
    ...
    # 2. Удаление строк от перекупов, длинных сообщений и списков с ценами
